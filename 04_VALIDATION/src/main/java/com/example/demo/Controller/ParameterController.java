...
public class ParameterController {
    ...
    //@RequestParam : 동기요청 파라미터 처리 / 그외 전부"html form" 기반 전달되는 파라미터 처리(JS의 form-data도 받기 가능, JSON Type 받기 불가)
    //@RequestBody  : 비동기요청 파라미터 처리 / "json,filedata"등 전달되는 파라미터 처리(html form 처리가능)
    @PostMapping("/p04")
...
//        session.setAttribute("dto",dto);
    ...
    public String redirect_init_handler(Model model, RedirectAttributes redirectAttributes){
        ...
        redirectAttributes.addFlashAttribute("r_init2","r_init_value2"); // session 속성추가
        return "redirect:/param/redirect/step1";
    }
    @GetMapping("/redirect/step1")
    public void redirect_step1_handler(Model model, @RequestParam String r_init){ // ( r_init2 생략 가능 )
...
//    }
    ...
    //@RequestParam, @RequsetBody, @ModelAttribute, @PathVariable
}
